...
const UpdateVisa = () => {
  ...
  useEffect(()=>{
    getAgencies()
    axios.get(`/api/visa/${parms.id}/`,getHeaders).then((res)=>{
  ...
  useEffect(()=>{
    methods.reset({...defaultContainer,agency:defaultContainer?.agency?.id,country:defaultContainer?.country?.code})

  },[defaultContainer])
  const onSubmit=(data)=>{
    data['title']=data['applicant_name']+" "+data['visa_type']+" visa"
    data['transaction_date']=data['remitted_date']
    axios.put(`/api/visa/${parms.id}/`,data,getHeaders).then((res)=>{
    updated()
    navigate("/visas")
    ...
    <FormProvider {...methods}>
      ...
      >
        <div className="registration__header">
          <h1>Visa Stambing</h1>
          <p>Update Your Visa Application</p>
        </div>
        ...
        <div className="registration__footer">
          <div className="registration__confirm">
            <button className="goback" onClick={()=>{navigate("/visas")}} >Go back</button>
